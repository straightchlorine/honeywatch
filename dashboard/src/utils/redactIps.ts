...
// anchor on `scheme://[userinfo@]` (any URI scheme - ftp://, tftp://, scp://,
// not just http(s)) and blot the host when it is a bare decimal
...
// `bs=1024`; the numeric value is range-checked below before it is blotted, so
// small numbers such as unix timestamps under the range pass through untouched.
...
// fragment. Dotted-quad IPV4 next. The scheme-qualified numeric-host URL rule
// fires for non-dotted hosts (decimal/hex/octal) that the IP rules can't see;
// the schemeless numeric-host rule is last since it is the most conservative
// (range-checked) fallback.
const IP_SOURCE = `(?:${BLOT_RE})|(?:${IPV6})|(?:${IPV4_STANDALONE})|(?:${URL_NUMERIC_HOST})|(?:${SCHEMELESS_NUMERIC_HOST})`
