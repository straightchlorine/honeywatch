...
def build_spec_dict(smorest_api: Api) -> dict[str, Any]:
    ...
    Arguments:
      smorest_api: the flask-smorest instance

    Returns:
      the OpenAPI spec as a dictionary

    Raises:
      RuntimeError: if spec is not initialized
    """
...
def dump_spec(spec: dict[str, Any], output: pathlib.Path) -> int:
    """Write OpenAPI spec to output with sorted keys and no `servers` block (keeps
    snapshot environment-independent). Arguments: spec: OpenAPI spec to write
    output: file path to write to Returns: bytes written"""
